...
struct PantallaBienvenida: View {
    ...
    // Equivalente al listOf() en Kotlin
    ...
    // Equivalente a var pasoActual by remember { mutableIntStateOf(0) }
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                let pantalla = pantallas[pasoActual]
                ...
                Button(action: {
                    if pasoActual == pantallas.count - 1 {
                        onboardingCompletado = true
                    } else {
                        withAnimation { pasoActual += 1 }
                    }
                }) {
                    Text(pasoActual == pantallas.count - 1 ? "⚙ Comenzar" : "Siguiente →")
                        .font(.headline).bold()
                        .foregroundColor(pasoActual == pantallas.count - 1 ? Color(hex: "#0D1B2A") : .white)
                        .frame(maxWidth: .infinity)
                        .frame(height: 52)
                        .background(pasoActual == pantallas.count - 1 ? Color(hex: "#F39C12") : Color(hex: "#2ECC71"))
                        .cornerRadius(12)
